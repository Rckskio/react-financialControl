...
export default function App() {
  ...
  useEffect(() => {
     ...
     * Título da aba
     */
    document.title = `Financial Control ${APP_VERSION}`;
  }, []);
  ...
  useEffect(() => {
    if (filterCategory.trim() === '') {
      ...
      const newFilteredTransactions = currentTransactions.filter(
        (transaction) => {
          return transaction.categoryLowerCase.includes(lowerCaseFilter);
        }
      );

      setFilteredTransactions(newFilteredTransactions);
    }
  }, [filterCategory, currentTransactions]);
  ...
  const handleFilterCategory = (filteredCategory) => {
  ...
  return (
    <div className="container">
      ...
      {currentTransactions.length === 0 && <Spinner>Please wait...</Spinner>}
